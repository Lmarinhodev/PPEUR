...
//Abrir modal selecionando pdf 
function openModal(caminho) {
  console.log(caminho)
  const modalElement = document.getElementById('ex1')
...
function handleHover(e, color) {
  const target =
      e.target.closest('[id]'); // Encontra o ancestral mais próximo com ID
  if (target && target !== destaque) { // Evita aplicar ao próprio #Destaque
    target.style.fill = color;
  }
}

destaque.addEventListener('mouseover', (e) => { handleHover(e, '#23238E'); });

destaque.addEventListener('mouseout', (e) => { handleHover(e, '#999999'); });
